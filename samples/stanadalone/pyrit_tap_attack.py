#!/usr/bin/env python3
"""Standalone TAP-style attack runner with colored logging and local-only prompt source."""
...
def _expand_tap_prompts(prompts: list[str], *, width: int) -> list[str]:
    templates = [
        "TAP branch A: {}",
        "TAP branch B: {}",
        "TAP branch C: {}",
    ...
    for prompt in prompts:
        for idx in range(max(1, width)):
            out.append(templates[idx % len(templates)].format(prompt))
    return out


async def _run_async(*, logger: DualLogger, prompts: list[str], dry_run: bool, max_seeds: int, width: int) -> int:
    prompts = prompts[:max_seeds] if max_seeds > 0 else prompts
    expanded = _expand_tap_prompts(prompts, width=width)

    if dry_run:
        logger.info(f"Dry-run mode enabled. Seed count={len(prompts)} expanded={len(expanded)}")
        for idx, prompt in enumerate(expanded[:5], start=1):
        ...
        _write_report(
            ...
            payload={
                ...
                "expanded_prompt_count": len(expanded),
                "sample_prompts": [p[:160] for p in expanded[:5]],
            },
        )
        return 0
...
def main() -> int:
    ...
    parser = argparse.ArgumentParser(description="Standalone TAP-style attack runner.")
    parser.add_argument("--dry-run", action="store_true", help="Validate prompt plan and output artifacts without model calls.")
    parser.add_argument("--max-seeds", type=int, default=0, help="Limit seed count (0 means all).")
    parser.add_argument("--width", type=int, default=2, help="Branching width per seed.")
    args = parser.parse_args()

    logger = DualLogger(script_name=SCRIPT_NAME)
    try:
        logger.info("Starting standalone TAP run")
        prompts, source = _load_prompts(logger)
        logger.info(f"Prompt source: {source}")
        return asyncio.run(_run_async(logger=logger, prompts=prompts, dry_run=bool(args.dry_run), max_seeds=args.max_seeds, width=max(1, args.width)))
