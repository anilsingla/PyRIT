...
# --- Ensure TAP attack API is importable ---
import sys
from pathlib import Path
script_dir = Path(__file__).resolve().parent
repo_root = script_dir.parent.parent.parent
redteam_runner_path = repo_root / "samples" / "security-evaluator" / "scripts" / "app"
sys.path.insert(0, str(redteam_runner_path))
...
# --- PyRIT 2.x+ API imports for TAP attack ---
...
MULTITURN_TAP_TURNS = 3                                   # number of multi-turn attack rounds

async def run_tap_attack():
    logger, tests, dataset_path, memory = await initialize_banking_attack_environment(
        script_name="pyrit_tap_attack", script_dir=script_dir, db_name="pyrit_tap.db"
    )
    ...
    for test in tests:
        ...
        for turn in range(1, MULTITURN_TAP_TURNS + 1):
            if turn == 1:
...
def main() -> int:
    try:
        return asyncio.run(run_tap_attack())
