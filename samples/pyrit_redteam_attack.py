"""Standalone RedTeam attack launcher with dual console/file logging.

This script mirrors the standalone Crescendo entrypoint pattern and delegates
execution to the security-evaluator RedTeam runner.
"""
...
def _resolve_repo_root() -> Path:
    ...
    defaults = [
        Path("c:/githubrepos/Anil_github_repos/PyRIT"),
        Path.cwd(),
    ]

    for base in defaults:
        if (base / "samples" / "security-evaluator").exists():
            return base.resolve()

    raise RuntimeError(
        "Could not locate PyRIT repo root. Set PYRIT_REPO_ROOT to the folder containing samples/security-evaluator."
    )
...
def main() -> int:
    writer, original_stdout, original_stderr = _setup_logging(prefix="banking_redteam_attack")
    planned_tests = 1
    executed_tests = 0
    passed_tests = 0
    failed_tests = 0
    try:
        ...
        runner = repo_root / "samples" / "security-evaluator" / "scripts" / "app" / "attacks" / "redteam_attack_runner.py"
        ...
        print("[INFO] Launching RedTeam runner")
